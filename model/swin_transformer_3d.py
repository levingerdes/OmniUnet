# Portions of the code under MIT License
# Copyright (c) 2025 Space Robotics Lab at UMA

# Code modified from 
# https://github.com/facebookresearch/omnivore/blob/main/omnivision/models/swin_transformer.py licensed under CC-BY-NC 4.0 license
# Previously modified from 
# https://github.com/SwinTransformer/Video-Swin-Transformer/blob/master/mmaction/models/backbones/swin_transformer.py licensed under Apache License 2.0
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, use_checkpoint=False, H=None, W=None, use_seg=False):
        ...
        feat_map = x
        feat_map = rearrange(feat_map, "b d h w c -> b c d h w")
        if self.downsample is not None:
            x = self.downsample(x)
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(
        ...
        self.standard_in_channels = 3 
        self.additional_channels  = max_channels - self.standard_in_channels
        
        ## We define the PatchEmbed tokenizers
        if self.depth_mode in ["summed_tokens"]:
            
            self.patch_embed = PatchEmbed3D(
                patch_size=self.patch_size,
                in_chans=self.standard_in_channels,
            ...
            if self.depth_mode is not None:
                ...
                for i in range(self.additional_channels):
                    aux_embed = PatchEmbed3D(
                        patch_size=self.patch_size,
                        in_chans=1,
                        embed_dim=self.embed_dim,
                        norm_layer=nn.LayerNorm,
                    )
                    setattr(self, f"add_patch_embed_{i}", aux_embed)
            else:
                logging.info("No add_patch_embed layer created")
        
        elif self.depth_mode in ["separate_tokens"]:
            msg = f"Using depth mode {self.depth_mode}"
            logging.info(msg)
        ...
        # Important part for pretraining
        # In pretraing case, additional calls for init_weights 
        # and free_stages will be made from outside
        self.init_weights()


    def _freeze_stages(self):
        """
        # There is no interest into freezing the tokenizer
    ...
    def get_patch_embedding(self, x, image_type):
        # While training, this function is used as a head that tokenizes inputs
        # x: B x C x T x H x W
        # We always assume the main training is with RGBDT for 
        # patch embed ubication
        
        # Image type by default, useful for
        # RGB, RGBD, and RGBDT
        if image_type == "":
            # Standard RGB channels as always presented standard
            input_dims = x.shape[1]
            additional_channels = input_dims - self.standard_in_channels
            # We define the function that merge tokens
            if self.depth_mode in ["summed_tokens"]:      
                x_rgb = x[:, :self.standard_in_channels, ...]
                x_rgb = self.patch_embed(x_rgb)

                if(input_dims > self.standard_in_channels):
                    x_add_list = []
                    for i in range(additional_channels):
                        x_add = x[:, self.standard_in_channels + i, ...]

                        ## Additional dimensions to use patch embed
                        x_add = torch.unsqueeze(x_add, 1)

                        ## We extract aux_embed function
                        aux_embed = getattr(self, f"add_patch_embed_{i}")
                        x_add = aux_embed(x_add)
                        x_add_list.append(x_add)
                        
                    # Sum of the sets of tokens
                    x = x_rgb
                    for i in range(len(x_add_list)):
                        x = x + x_add_list[i]
                else:
                    # Entering RGB image
                    x = x_rgb
        ...
        # Image_type Thermal
            ...
            # Index 1 of patch embed for thermal
            aux_embed = getattr(self, f"add_patch_embed_{1}")

            # Additional dimensions to use patch embed
            x_input   = torch.unsqueeze(x_input, 1)
            x_thermal = aux_embed(x_input)
            
            # Entering thermal image
            x = x_thermal
        ...
        # Image_type Depth
            ...
            # Index 0 of patch embed for depth
            aux_embed = getattr(self, f"add_patch_embed_{0}")

            # Additional dimensions to use patch embed
            x_input   = torch.unsqueeze(x_input, 1)
            x_depth   = aux_embed(x_input)
    ...
    def forward(
        ...
        x = self.get_patch_embedding(x, image_type)
        ...
        for layer in self.layers:
            [x,feat_map] = layer(x.contiguous(), use_checkpoint=use_checkpoint)
