...
def train_net(net, device, epochs: int = 5,  batch_size:  int = 1,    learning_rate: float = 1e-5,
              val_percent: float = 0.2, save_checkpoint: bool = True, amp: bool = False, dataset_str: str = '', 
    ...
    if concat == True:
        ...
        # Always using same manual seed for reproducibility and testing purposes 
        train_set, val_set = random_split(dataset, [n_train, n_val], generator = torch.Generator().manual_seed(0))
            ...
            for batch in train_loader:
                ...
                with torch.cuda.amp.autocast(enabled = amp):
                    if dataset_str == 'T' or dataset_str == 'D':
                        masks_pred, _ = net(images, image_type = dataset_str)
                    else:
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        ...
                        pixel_acc_score, IoU_score, val_dice_score, precision_score, recall_score, validation_loss = utils.evaluate_net.evaluate(net, val_loader, device, masks_pred_size, dataset_str)
...
def get_args():
    ...
    parser.add_argument('--validation',       '-v', type = float, default = 20.0,  dest='val',  help='Percent of the data that is used as validation (0-100)')
    ...
    parser.add_argument('--frozen_stages',    '-g', type = int,   default = -1,    help='Frozen stages')
...
if __name__ == '__main__':
    ...
    if args.load:
        net = OmniUnet(max_channels = args.max_channels, n_classes = args.pretrain_classes, bilinear = args.bilinear, depth_mode = depthmode, pretrained = True, frozen_stages = args.frozen_stages)
        logging.info(f'Network: {args.frozen_stages} frozen stages')

        # Load the model once weights init
        loaded_model  = torch.load(args.load, map_location = device)
        net.load_state_dict(loaded_model, strict = True)
        # To freeze the sathes of the loaded model
        net.trunk._freeze_stages()
